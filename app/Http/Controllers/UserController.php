...
//                           ON movie_videos.id = payments.movie_id');
    ...
    public function show_price($id){
        if(!Auth::check())
        {
            ...
            \Session::flash('error_flash_message', trans('Please login'));
            ...
            if ($payment == null) {
                if (!Auth::check()) {
                    ...
                    \Session::flash('error_flash_message', trans('words.Please Login'));
...
//        $plan_list = SubscriptionPlan::find($id);
    ...
    public function payment_save(Request $request){
        ...
        if($request->plan=='day'){
            $data->expiry_date= date('Y-m-d H:i:s', strtotime("+1 day"));

        }
        if($request->plan=='month'){
            $data->expiry_date= date('Y-m-d H:i:s', strtotime("+30 day"));

        }
        if($request->plan=='year'){
            $data->expiry_date= date('Y-m-d H:i:s', strtotime("+360 day"));

        }
        if($request->plan=='lifetime'){
            $data->expiry_date= date('Y-m-d H:i:s', strtotime("+3600 day"));
